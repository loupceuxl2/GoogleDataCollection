        ...
        public static async Task<int> RunDataCollector(DataContainer data)
        {
            var prioritisedUpdates =
                data.Edges.GroupBy(e => e.Updates.Count, e => e,
                    ...
                    .SelectMany(x => x.Edges.Select(y => new Tuple<int, Edge, UpdateTime>(x.UpdateCount, y, data.UpdateTimes[x.UpdateCount % data.UpdateTimes.Count])))                 // Flatten groups into a list of Tuples<int, Edge> | Tuples<UpdateCount, Edge>.
                    .ToList();
